...
// File contracts/JesusPunksDNA.sol

//SPDX-License-Identifier: MIT

pragma solidity ^0.8.0;

contract JesusPunksDNA{
...
contract JesusPunks is ERC721, ERC721Enumerable, JesusPunksDNA{
    ...
    constructor(uint256 _maxSupply) ERC721("JesusPunks", "JPNKS") {
        maxSupply = _maxSupply;
    }

    function mint() public{
        uint256 current = _idCounter.current();
        require(current < maxSupply, "Jesus Punks are no longer available");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "Jesus Punks #',
                tokenId.toString(),
                '", "description": "Jesus Punks are random avatars created and stored in my DApp developer training", "image": "',
                image,
                '"}'
            )
        );
